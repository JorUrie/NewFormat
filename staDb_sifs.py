...
def SiteVecs(Arch_C, Arch_D):
    ...
    # -----------------------------------------------------------------------------------------------------------------
    '''Ocurrio un problema muy dificil de resolver, hay dos estaciones (SLAC y SLCR) que se repiten, pero tienen 
    diferentes propiedades. Al no poder calcular su indice sin considerar esta repeticion, se ha decidido dividir la
    lista de las estaciones, calcular su indice para poder repetirlo las veces necesarias. Afortunadamente conocemos las
    posiciones de las estaciones repetidas. Una vez hecho lo mencionado anteriormente se unen ambas listas.
    A continuacion se divide la lista de las estaciones con los indices de las fechas sustituidas por un [*]'''
    Sta_sis_1 = Sta_sis[:249]
    Sta_sis_2 = Sta_sis[249:]
    Buch_1 = Buch[:78]
    Buch_2 = Buch[78:]
    # -----------------------------------------------------------------------------------------------------------------
    '''Se busca el indice de cada una delas estaciones ya partidas las listas. 
    ...
    St_1 = []
    St_2 = []
    for i in range(len(Buch_1)):
        Sta_1 = Sta_sis_1.index(Buch_1[i])
        St_1.append(Sta_1+1)
    for j in range(len(Buch_2)):
        Sta_2 = Sta_sis_2.index(Buch_2[j])
        St_2.append(Sta_2 + St_1[-1] + 2)
    '''IMPORTANTE: Como se mencionadaba anterioremente se ha dividido de dos la lista debido a que dos estaciones
    se repetian y cuando se buscaba sus indices solo colocaba el primero que encontraba (Python) para solucionar 
    este problema hemos divido en dos la lista, antes de que aparezca las estaciones repetidas. Por eso debemos ser 
    cuidadosos de las cantidades que se van a sumar. La lista se ha dividido antes de que aparezca el numbre de la
    estacion, es decir en una fecha. Se ha sumado una unidad para que inicie desde 1 y no desde ceroy sea mas facil 
    calcular las veces que se debe repetir la estacion, depuees se suman dos unidades porque (en este caso) es donde
    se encuentra la siguiente estacion por ejemplo, la primera lista comineza en cero y termina en 247, pero se suma 
    una unidad, entonces comienza en uno y termina en 248, sin embargo aun hay mas columnas y la siguiente estacion
    ya no se encuentra dsiponible pero sabemos que hay una fecha. 
    Para la segunda lista hemos sumando la ultima cantidad de la primera lista y como debemos recorrer la posicion y 
    como sabemos que entre ambas estaciones hay un espacio entonces se sumas dos unidades y con eso la lista 
    tiene continuidad por lo que resta juntar ambas listas'''
